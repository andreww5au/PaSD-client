...
class SimFNDH(fndh.FNDH):
    ...
    def sim_loop(self):
        ...
        while not self.wants_exit:  # Process packets until we are told to die
            ...
            self.psu48v1_voltage = random_walk(self.psu48v1_voltage, 48.1, scale=0.1)
            self.psu48v2_voltage = random_walk(self.psu48v2_voltage, 48.1, scale=0.1)
            self.psu5v_voltage = random_walk(self.psu5v_voltage, 5.1, scale=0.025)
            self.psu48v_current = random_walk(self.psu48v_current, 13.4, scale=0.5)
            self.psu48v_temp = random_walk(self.psu48v_temp, 58.3, scale=0.5)
            self.psu5v_temp = random_walk(self.psu5v_temp, 55.1, scale=0.5)
            self.pcb_temp = random_walk(self.pcb_temp, 48.1, scale=0.25)
            self.outside_temp = random_walk(self.outside_temp, 38.1, scale=0.5)
