...
#######################################
    ...
    s.conn.writeMultReg(modbus_address=1, regnum=10001, valuelist=[crc32 & 0xffff, crc32 >> 16])
    s.conn.writeReg(modbus_address=1, regnum=10125, value=1, timeout=10.0)                 
    print("return code: " + str(s.conn.readReg(modbus_address=1, regnum=10126)[0][1]))  # least sig byte
...
#                    print(hex(registerBytes[i * 2] + (registerBytes[i * 2 + 1] << 8)))
                ...
                if length < 320:
                    ...
                    s.conn.writeReg(modbus_address=1, regnum=10125, value=2)  # write command                 
                else:
...
#                    regValues.append(2)
    ...
    s.conn.writeMultReg(modbus_address=1, regnum=10001, valuelist=regValues)
    s.conn.writeReg(modbus_address=1, regnum=10125, value=3)  # trust but verify

    verifyResult = s.conn.readReg(modbus_address=1, regnum=10126)[0][1]
    if verifyResult == 0:
        ...
        regValues=[crc32 & 0xffff, crc32 >> 16]
        s.conn.writeMultReg(modbus_address=1, regnum=10001, valuelist=regValues)
        s.conn.writeReg(modbus_address=1, regnum=10125, value=4)  # update
        updateResult = s.conn.readReg(modbus_address=1, regnum=10126)[0][1]
        if updateResult == 0:
