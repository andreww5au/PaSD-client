...
#######################################
    ...
    print("Issuing sample state command...")

    s.conn.writeReg(modbus_address=1, regnum=10125, value=9)  # sample state command

    result = s.conn.readReg(modbus_address=1, regnum=10126)[0][1]  # results register
    if result == 0:
        print("Result ok.")

        sampleState = s.conn.readReg(modbus_address=1, regnum=10127)[0][1]
        print("sample state: " + str(sampleState) + "; 0 = STOPPED, 1 = SAMPLING")
